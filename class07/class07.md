...
- [PCA of UK food data](#pca-of-uk-food-data)
  ...
  - [Pairsplots and heatmaps](#pairsplots-and-heatmaps)
...
Today we will begin our explortion of important machine learning methods
with a focus on **clustering** and **dimensionallity reduction**.
...
> **Q.** Can you generate 30 numbers centered at +3 and 30 at -3 taken
...
The main function in “base R” for k-means clustering is `kmeans()`. The
function takes two main arguments: the data to be clustered and the
...
> **Q.** What component of your kmeans result object has the cluster
> size(ie. how many points are in each cluster)?
...
> **Q.** What component of your kmeans result object has the cluster
> member vector(ie. the main clustering result)?
...
> **Q.** Plot the results of clustering (ie. out data colored by the
> clustering result)
...
> **Q.** Run kmeans again and cluster into 4 clusters and plot the
> results.
...
> **Key Point**: K-means clustering always return the clustering that we
> ask for (this is the “K” or “centers” in K-means). It is up to the
> user to decide what K is appropriate for their data.

## Hierarchical Clustering

The main functions in “base R” for hierarchical clustering are `dist()`
and `hclust()`. The `dist()` function computes the distance matrix
between all pairs of points in the data, while the `hclust()` function
...
that you can’t just pass your input data directly to `hclust()`, you
need to first compute the distance matrix with `dist()`.
...
PCA is a popular dimensionallity reduction technique that transforms a
dataset into a new coordinate system such that the greatest variance by
any projection of the data comes to lie on the first coordinate (called
...
I like the method where we fix the column to the row names as we read in
the data. This prevents us from having to do an extra step after reading
in the data. Furthermore, we dont accidentally delete any columns if we
rerun the code.
...
Using `beside = FALSE` in the barplot function results in the following
plot.

Modify the data to make it plottable with ggplot. We are changing from
wide to long format using the `pivot_longer()` function from the
`tidyverse` package.
...
If a point is on the diagonal for a given plot, it means that the
consumption of that food item is the same in both countries being
compared.
...
From the plots we see that England, Wales, and Scotland cluster
together, while Northern Ireland forms its own cluster. We see that
Northern Ireland has higher consumption of fresh potatoes and soft
drinks compared to the rest of the UK. Of all of the plots, `pairs()`
was the most useful. It did take a bit of work to interpret and will be
even more difficult with larger datasets.

### PCA to the rescue!

The main base R function for PCA is `prcomp()`. The function takes the
data to be analyzed as its main argument.
...
I need 2 principal components (PCs) to capture at least 90% of the
variance.
...
How do the original variables (countries) contribute to the new PCs? We
can look at the loadings of each variable on each PC.
...
The two food groups that feature predominantly are softdrinks and fresh
potatoes. In PC2, positive scores associate with Scotland, near zero
scores associate with England, and negative scores associate with Wales
and Northern Ireland. PC2 contrasts drink heavy diets vs. staple/produce
heavy diets. From PC2, we can assign Scotland to soft drinks and Wales
to Fresh potatoes.
